    ...
    while true do
        ...
        if _G.AutoCoins and lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if coins then
                ...
                if target and target.Parent and hum then
                    -- Use normal Humanoid movement instead of changing CFrame.
                    -- This lets Roblox/server physics handle the character position.
                    local movePos = target.Position - Vector3.new(0, 3.2, 0)
                    hum:MoveTo(movePos)

                    local started = tick()
                    repeat
                        task.wait(0.05)
                        if not _G.AutoCoins or not target.Parent then break end
                    until (hrp.Position - movePos).Magnitude <= 3 or tick() - started >= 3

                    -- Only attempt the touch once the character has actually walked there.
                    if target.Parent and _G.AutoCoins and (hrp.Position - target.Position).Magnitude <= 5 then
