...
-- Redesigned Visual Tokens
local ExodusBlue = Color3.fromRGB(0, 180, 255)
local ExodusDarkBg = Color3.fromRGB(13, 15, 20)
local ExodusCardBg = Color3.fromRGB(20, 23, 31)
...
--------------------------------------------------------------------------------
-- INTEGRATED STANDALONE INVISIBILITY SCRIPT
--------------------------------------------------------------------------------
...
StandaloneInvisStroke.Color = Color3.fromRGB(255, 255, 255)
StandaloneInvisStroke.Transparency = 0.85
StandaloneInvisStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
StandaloneInvisStroke.Parent = StandaloneInvisBtn

-- Local Invisibility Modifier Function
local function SetCharacterInvisibility(character, state)
...
StandaloneInvisBtn.MouseButton1Click:Connect(function()
    InvisibilityActive = not InvisibilityActive
    
    if InvisibilityActive then
        TweenService:Create(StandaloneInvisBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(45, 195, 115)}):Play()
        StandaloneInvisBtn.Text = "INVIS: ON"
        SetCharacterInvisibility(lp.Character, true)
    else
        TweenService:Create(StandaloneInvisBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(220, 50, 65)}):Play()
        StandaloneInvisBtn.Text = "INVIS: OFF"
        SetCharacterInvisibility(lp.Character, false)
    end
end)
...
lp.CharacterAdded:Connect(function(char)
    ...
    task.wait(0.1)
    if InvisibilityActive then
        SetCharacterInvisibility(char, true)
    end
end)
--------------------------------------------------------------------------------
...
local function CreateFloatingButton(config)
    ...
    local textSize = config.TextSize or (isRect and 15 or 14)
    local cornerRadius = config.CornerRadius or (isRect and UDim.new(0, 10) or UDim.new(1, 0))
    local bgColor = config.BgColor or ExodusDarkBg
    local bgTransparency = config.BgTransparency or 0.12
    local textColor = config.TextColor or Color3.fromRGB(255, 255, 255)
    local strokeColor = config.StrokeColor or ExodusBlue
    local strokeThickness = config.StrokeThickness or 2
    local strokeTransparency = config.StrokeTransparency or 0.25
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.02, TextTransparency = 0}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.2), {Thickness = strokeThickness + 1, Transparency = 0}):Play()
    end)

    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundTransparency = bgTransparency, TextTransparency = 0.05}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.2), {Thickness = strokeThickness, Transparency = strokeTransparency}):Play()
    end)
...
local function CreateStatusBox(config)
    ...
    box.Size = size
    box.Position = pos
    box.BackgroundColor3 = ExodusDarkBg
...
local InvisBtn = CreateFloatingButton({
    ...
    Visible = false,
    TextSize = 12,
    StrokeColor = Color3.fromRGB(220, 50, 65),
    StrokeThickness = 2,
...
XrayBtn.Size = UDim2.new(0, 95, 0, 32)
XrayBtn.Position = XrayDefaultPos
XrayBtn.BackgroundColor3 = ExodusDarkBg
XrayBtn.BackgroundTransparency = 0.12
XrayBtn.Text = "X-RAY: 🔴"
XrayBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
XrayBtn.Font = Enum.Font.GothamBold
XrayBtn.TextSize = 12
XrayBtn.TextTransparency = 0.05
XrayBtn.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
XrayBtn.TextStrokeTransparency = 0.6
XrayBtn.AutoButtonColor = false
...
XrayStroke.Color = Color3.fromRGB(220, 50, 65)
XrayStroke.Transparency = 0.25
XrayStroke.Parent = XrayBtn

XrayBtn.MouseEnter:Connect(function()
    TweenService:Create(XrayBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.02, TextTransparency = 0}):Play()
    TweenService:Create(XrayStroke, TweenInfo.new(0.2), {Thickness = 3}):Play()
end)

XrayBtn.MouseLeave:Connect(function()
    TweenService:Create(XrayBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.12, TextTransparency = 0.05}):Play()
    TweenService:Create(XrayStroke, TweenInfo.new(0.2), {Thickness = 2}):Play()
end)
...
XrayBtn.MouseButton1Click:Connect(function()
    ...
    if XrayState then
        XrayBtn.Text = "X-RAY: 🟢"
        XrayStroke.Color = Color3.fromRGB(45, 195, 115)
    else
        XrayBtn.Text = "X-RAY: 🔴"
        XrayStroke.Color = Color3.fromRGB(220, 50, 65)
    end
...
function ToggleGhostMode()
    ...
    if InvisState then
        InvisBtn.Text = "INVIS: 🟢"
        InvisBtn.UIStroke.Color = Color3.fromRGB(45, 195, 115)
    else
        InvisBtn.Text = "INVIS: 🔴"
        InvisBtn.UIStroke.Color = Color3.fromRGB(220, 50, 65)
    end
...
local backpack = p:FindFirstChild("Backpack")
                    ...
                    if hasKnife or role == "Murderer" then
                        local mColor = Color3.fromRGB(245, 55, 75)
                        highlight.FillColor = mColor
                        label.Text = "MURDERER\n▼"
                        label.TextColor3 = mColor
                    elseif hasGun then
                        local sColor = Color3.fromRGB(0, 180, 255)
                    ...
                    elseif role == "Sheriff" or role == "Hero" then
                        if gunDropped or (activeHero and activeHero ~= p) then
                            local iColor = Color3.fromRGB(45, 200, 115)
...
-- AutoFarm movement is handled by Humanoid:MoveTo; do not override
-- character velocity/state here, so normal movement can occur.
...
task.spawn(function()while true do
        ...
        if drop and not drop:FindFirstChild("ExodusGunESP") then
            ...
            label.TextSize = 15
            label.Text = "GUN HERE\n▼"
            label.TextColor3 = Color3.fromRGB(45, 200, 115)
            label.TextStrokeTransparency = 0.2
...
task.spawn(function()
    ...
    local function getNearestCoin(root, coins)
        ...
        for _, c in ipairs(coins:GetChildren()) do
            if c:IsA("BasePart")
                and c.Name == "Coin_Server"
                and c:FindFirstChild("TouchInterest")
                and c:FindFirstChild("CoinVisual")
                and not collectedCoins[c]
                and not c:GetAttribute("Collected") then
    ...
    while true do
        ...
        if _G.AutoCoins and lp.Character then
            local char = lp.Character
            local hrp = char:FindFirstChild("HumanoidRootPart")
            local hum = char:FindFirstChildOfClass("Humanoid")
            local coins = workspace:FindFirstChild("CoinContainer", true)
                or workspace:FindFirstChild("Coins", true)

            if hrp and hum and coins then
                if not layTrack or not layTrack.IsPlaying then
                ...
                if not targetCoin
                    or not targetCoin.Parent
                    or targetCoin:GetAttribute("Collected")
                    or collectedCoins[targetCoin] then
                    targetCoin = getNearestCoin(hrp, coins)
                end

                if targetCoin then
                    local targetPos = targetCoin.Position - Vector3.new(0, 3.2, 0)
                    local offset = targetPos - hrp.Position
                    local distance = offset.Magnitude

                    if distance > 2.5 then
                        local direction = offset.Unit
                        hrp.AssemblyLinearVelocity = direction * math.min(farmSpeed, math.max(distance * 2, 4))
                    else
                        hrp.AssemblyLinearVelocity = Vector3.zero
...
local container = getCoinContainer()
        if container and not isInLobby() then
            ...
            if targetCoin then
                local subPos = targetCoin.Position - Vector3.new(0, 3.2, 0)
                hrp.CFrame = CFrame.new(subPos)
                hrp.AssemblyLinearVelocity = Vector3.zero
                hrp.AssemblyAngularVelocity = Vector3.zero
                if targetCoin and targetCoin.Parent and _G.SafeAutoFarm then
                    firetouchinterest(hrp, targetCoin, 0)
                    task.wait(0.05)
                    firetouchinterest(hrp, targetCoin, 1)
                end
